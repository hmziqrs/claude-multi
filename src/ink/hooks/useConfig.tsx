...
export function useConfig() {
  ...
  const removeInstance = useCallback(
    async (name: string) => {
      ...
      if (!instance)
        throw new ClaudeMultiError(ErrorCode.INSTANCE_NOT_FOUND, `Instance '${name}' not found`);
  ...
  const toggleSyncMode = useCallback(
    async (name: string, newMode: SyncModeType) => {
  ...
  const syncTemplateEnv = useCallback(
    async (instance: Instance) => {
  ...
  const regenerateWrapper = useCallback(
    async (instance: Instance) => {
      await createWrapper({
        name: instance.name,
