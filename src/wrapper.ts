...
/**
 * Generates a shell wrapper script that sets CLAUDE_CONFIG_DIR
 */
 ...
 * Uses npm global prefix on Windows
 */
export function getDefaultBinaryPath(name: string): string {
  let binDir: string;
  if (process.platform === "win32") {
    // Use npm's global prefix (where npm installs bins)
    try {
      binDir = execSync("npm prefix -g", { encoding: "utf-8" }).trim();
    } catch {
      // Fallback to APPDATA\npm
      binDir = resolve(process.env.APPDATA || "", "npm");
    }
  } else {
